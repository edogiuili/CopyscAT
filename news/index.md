...
- Added five functions for inspecting per-cell sequencing depth before
  running the CNV workflow:
  - [`signalPerCell()`](https://edogiuili.github.io/CopyscAT/reference/signalPerCell.md)
    summarises the signal each barcode carries, from matrix files or
    in-memory data frames, across any number of samples.
  - [`fragmentsPerCell()`](https://edogiuili.github.io/CopyscAT/reference/fragmentsPerCell.md)
    counts fragments per barcode from a 10x `fragments.tsv.gz` or a
    CellRanger `singlecell.csv`.
  - [`summariseCellDepth()`](https://edogiuili.github.io/CopyscAT/reference/summariseCellDepth.md)
    returns per-sample quartiles.
  - [`plotCellDistribution()`](https://edogiuili.github.io/CopyscAT/reference/plotCellDistribution.md)
    draws faceted log-scale histograms, with an optional line at a
    proposed cutoff.
  - [`plotCellKnee()`](https://edogiuili.github.io/CopyscAT/reference/plotCellKnee.md)
    draws ranked barcode curves, whose elbow marks the transition from
    cells to background.

  Note that
  [`signalPerCell()`](https://edogiuili.github.io/CopyscAT/reference/signalPerCell.md)
  reports base pairs, not fragments: `process_fragment_file.py`
  accumulates fragment *lengths*, so the row sum of a CopyscAT matrix is
  the genomic span a cell covers. The two functions therefore return
  differently named columns, `signal_bp` and `n_fragments`, and the
  plotting functions use whichever is present.
...
- [`annotateCNV4B()`](https://edogiuili.github.io/CopyscAT/reference/annotateCNV4B.md)
  failed with “`condition` must be a logical vector, not a logical
  matrix” on current dplyr. [`t()`](https://rdrr.io/r/base/t.html) on
  the one-row normal-cluster frame returns an n-by-1 matrix; older dplyr
  collapsed it inside `mutate()`, newer versions do not. The value is
  now converted with
  [`as.vector()`](https://rdrr.io/r/base/vector.html), and a guard
  reports a mismatch between the number of cluster indices and
  chromosome arms.
- [`normalizeMatrixN()`](https://edogiuili.github.io/CopyscAT/reference/normalizeMatrixN.md)
  let a barcode with no signal reach
  [`edgeR::cpm()`](https://rdrr.io/pkg/edgeR/man/cpm.html), which
  rejected it with “library sizes should be greater than zero” — naming
  neither the barcode nor the parameter at fault. This happens whenever
  `maxZero` is at or above the bin count, which disables the empty-bin
  filter. The new check lists the affected barcodes, the retained and
  blacklisted bin counts, whether the filter is disabled, and the three
  ways to resolve it.
...
- Added a testthat suite of 122 tests, including end-to-end integration
